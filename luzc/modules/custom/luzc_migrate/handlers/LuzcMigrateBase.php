...
/**
 * @file
 * Contains \LuzcMigration.
 */

abstract class LuzcMigrateBase extends Migration {
  ...
  public function __construct($arguments) {
    ...
    if (!in_array($this->entityType, array('node', 'taxonomy_term'))) {
      throw new Exception('\LuzcMigration supports only nodes and terms.');
    }
  ...
  protected function getMigrateDirectory() {
    return variable_get('luzc_migrate_directory', FALSE) ? variable_get('luzc_migrate_directory') : drupal_get_path('module', 'luzc_migrate');
  }
}
